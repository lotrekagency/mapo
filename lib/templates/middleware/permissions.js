...
Middleware.permissions = Middleware.permissions || async function ({ route, error, store }) {
    ...
    if (userInfo.is_superuser) {
        await store.dispatch('bossa/user/addPagePermission', {key: route.path, value: ["add", "change", "delete", "view"]})
        return
    }
    ...
    if (!userPermission.includes('view')) {
        ...
        await store.dispatch('bossa/user/addPagePermission', {key: route.path, value: userPermission})
        return
    }
}