...
export default async function BossaModule(moduleOptions) {
    ...
    // INJECT AXIOS MODULE
    this.requireModule(['@nuxtjs/axios', (this.options && this.options.axios) || userModuleOptions.axios || bossaDefaults.axios || {}])

    // INJECT INTERCEPTOR
    this.addPlugin({
        src: path.resolve(__dirname, './templates/api/core/interceptor.js'),
        fileName: path.join('bossamodule', 'plugins.interceptor.js')
    })

    // INJECT BOSSAPLUGIN
    this.addPlugin({
        src: path.resolve(__dirname, './templates/plugins/bossa.plugin.js'),
        fileName: path.join('bossamodule', 'plugins.bossa.js')
    })
